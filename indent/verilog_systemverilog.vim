...
setlocal indentkeys=!^F,o,O,0),0},=begin,=end,=join,=endcase,=join_any,=join_none,=generate,=module
...
function GetVerilog_SystemVerilogIndent()
  ...
  " Indent after a 'generate' statement
  elseif last_line =~ '\(\<generate\>\)\(\s*:\s*\w\+\)*' . vlog_comment . '*$' &&
    \ last_line !~ '\(//\|/\*\).*\(\<generate\>\)' &&
    \ ( last_line2 !~ vlog_openstat . '\s*' . vlog_comment . '*$' ||
    \ last_line2 =~ '^\s*[^=!]\+\s*:\s*' . vlog_comment . '*$' )
    let ind = ind + offset
    if vverb | echo vverb_str "Indent after generate statement." | endif
  ...
  elseif last_line =~ '[{(]' . vlog_comment . '*$' &&
	...
	" Indent after a ':' at the end of line
  elseif last_line =~ '\w\+:\s*' . vlog_comment . '*$' &&
    \ last_line !~ '\(//\|/\*\).*\w\+:\s*' &&
    \ ( last_line2 !~ vlog_openstat . '\s*' . vlog_comment . '*$' ||
    \ last_line2 =~ '^\s*[^=!]\+\s*:\s*' . vlog_comment . '*$' )
    let ind = ind + offset
	if vverb | echo vverb_str "Indent after : at the end of line" last_line | endif
  ...
  if curr_line =~ '^\s*\<\(join\|join_any\|join_none\|\|end\|endcase\|endgenerate\|while\)\>' ||
  ...
  elseif curr_line =~ '^\s*\<begin\>'
	if last_line !~ '^\s*\<\(function\|task\|specify\|module\|class\|package\)\>' &&
	  ...
	  let ind = ind - offset
	  if vverb
	echo vverb_str "De-indent a stand alone begin statement." last_line
	  endif
	endif
